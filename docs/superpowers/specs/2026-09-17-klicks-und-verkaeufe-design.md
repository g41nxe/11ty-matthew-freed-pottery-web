...
| Platzierung (`utm_content`) | **Ort, nicht Stück:** `nav`, `hero`, `about-medal`, `redirect-shop`, `home-teaser-<slug>`, `collections-<slug>`, `firing-shop-all`, `firing-<set>-featured`, `firing-<set>-tile1` bis `tile4`, dazu `text-<seite>` für Shop-Links, die Matthew in ein Textfeld schreibt (`text-events`, `text-home`). Welches Stück gemeint war, weiß Shopify aus der Landeseite. Stückname wäre lesbarer, bricht aber die Historie, sobald Matthew ihn in Tina ändert (entschieden 2026-09-18) |
| Wo die Parameter entstehen | In einem Eleventy-Filter zur Bauzeit, **nicht im CMS**. Matthew tippt nie Parameter; die URLs in Tina bleiben sauber. Was kein Template benennt, fängt ein Transform nach dem Rendern auf: Er taggt jeden noch ungetaggten Shop-Link einer Seite als `text-<seite>`. Als Shop gelten `shop.…` und `…myshopify.com` (ergänzt 2026-09-19) |
