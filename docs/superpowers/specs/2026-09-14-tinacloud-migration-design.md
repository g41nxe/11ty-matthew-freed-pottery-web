...
| Shop-Sets | `home.md` hält `shop_sets`: je Set `name` (nur im CMS), `hide`, `featured_piece` und bis zu vier `items` (ohne eigenes `hide`). Die Startseite zeigt das erste nicht versteckte Set; ein Wechsel pro Seitenaufruf ist geplant, das Backend ist dafür bereit. Sets: Tofino (sichtbar) und Yaletown (versteckt, Featured Piece ist ein Entwurf aus der Galerie). Die übrigen acht Artikel entfallen und bleiben in der Git-Historie (2026-09-16) |
