...
| Galerie, Features | `showcase.json` getrennt: `gallery.json` bleibt eigene Datei (Collections-Seite und Home lesen sie), die Shop-Artikel ziehen ins Frontmatter von `home.md` (2026-09-16), dort als Shop-Sets (siehe unten) |
| Shop-Sets | `home.md` hält `shop_sets`: je Set `name` (nur im CMS), `hide`, `featured_piece` und bis zu vier `items`. Die Startseite zeigt bei jedem Aufruf ein zufälliges Set. Sechs Sets: Tofino, Yaletown, Joffre, Ölflaschen, Tree of Life, Strathcona (2026-09-17) |
| Namen im CMS | Jede Collection beginnt mit der Seite, auf der ihr Inhalt erscheint: „Events · Markets and events“, „About · Art page“ usw., interne Namen und Dateien entsprechend (`events_markets`, `eventsMarkets.ts`). Die Einstellungen für alle Seiten heißen nur „Settings“ (2026-09-16) |
| Tina-Version | `tinacms` 3.14.0 und `@tinacms/cli` 3.0.0 exakt gepinnt, ohne `overrides` (2026-09-16) |
...
- **Tina 3.14 und Umbenennung (2026-09-16):**
  - **Upgrade:** TinaCloud beantwortet die Schnittstelle 3.0 inzwischen (Test mit dem Content-Token: gleiche Antwort wie 2.4). Grund zum Hochziehen: 3.13 bringt `react-final-form` 7.0.1 mit, das ungespeicherte Änderungen in verschachtelten Listen verwirft, sobald man zurücknavigiert. Betroffen wären Markets mit Terminen, Events, FAQ, Abschnitte, Händler und Shop-Sets. 3.14 geht zurück auf `final-form` 4. Im Admin nachgewiesen: Preis in einem Shop-Artikel geändert, zwei Ebenen zurück, wieder geöffnet, Änderung noch da. Der Bruch in `@tinacms/graphql` 3.0 betrifft nur die erzeugten TypeScript-Typen für Rich-Text; die nutzen wir nicht.
  - **Uploads:** `media.accept` erlaubt nur JPEG, PNG und WebP. `sharp` liest HEIC nicht (HEIF nur als AVIF), ein iPhone-Foto würde den Build brechen.
  - **Leerzeile im Datenschutz-Body:** Tina schreibt vor einen Text-Body einen Zeilenumbruch, liest ihn aber über `gray-matter` als Teil des Bodys zurück. Jedes Speichern fügte eine Leerzeile oben an (auch schon unter 3.13, siehe Commit 0a10e16). `beforeSubmit` in der Collection entfernt führende Leerzeilen; zwei Speichervorgänge im Admin lassen die Datei unverändert. Der Round-Trip über GraphQL umgeht `beforeSubmit` und fügt die Zeile weiter an; er vergleicht den Body getrimmt. Lokal kommt bei `core.autocrlf` ein gemischtes Zeilenende dazu; im Repo steht nur LF.
  - **Pottery-Seite:** `/about/pottery.html` („Art“, „Everyday art“) war seit dem Redesign im Juli nirgends verlinkt, weil die Reiterleiste unter About entfiel. Jetzt verlinkt der Footer sie („Everyday art“), und alle drei About-Seiten zeigen am Ende Karten zu den jeweils anderen beiden (Bild aus dem ersten Abschnitt, Eyebrow, Überschrift; Überschrift „Keep reading“ in `global.labels`).
  - **Prüfung:** Round-Trip über alle 13 Collections unter den neuen Namen, `npm test` und `tsc` grün. Seitenvergleich: Startseite identisch bis auf Leerraum; geändert sind nur die drei About-Seiten (Karten) und der Footer aller Seiten.
- **Shop-Sets und Zufallswechsel (2026-09-17):** Sechs Sets in `home.md`, jedes mit Featured Piece und vier Artikeln. Die Bilder stammen aus Matthews Shop (erste Produktaufnahme, auf 1200×900 mit weißem Rand beschnitten) und aus `_archive`; Texte und Preise sind Entwürfe nach den Shop-Beschreibungen, mehrere Artikel sind derzeit ausverkauft.
  - **Wechsel pro Seitenaufruf:** Alle Sets stehen im HTML, alle mit der Klasse `hidden`; ein Inline-Skript direkt hinter den Abschnitten nimmt die Klasse bei einem zufälligen Set weg. Ohne JavaScript zeigt eine `noscript`-Regel das erste Set.
  - **Warum Klasse statt `hidden`-Attribut:** Die Browser-Regel für `[hidden]` ist `!important`, kein Stylesheet holt ein Set zurück — die `noscript`-Rückfallebene wäre wirkungslos.
  - **Warum alle Sets versteckt starten:** Ein sichtbarer Abschnitt lädt seine Bilder, bevor das Skript ihn wieder verstecken kann (gemessen: drei Bilder). So lädt der Browser nur die fünf Bilder des gezeigten Sets.
  - **Ein Bild lädt sonst nie:** Ein `loading="lazy"`-Bild, das beim Laden `display:none` war, bleibt auch nach dem Einblenden ungeladen. Das Skript setzt deshalb `src` der fünf Bilder neu.
  - **Vergleichsseite:** `/preview-sets.html` zeigt alle Sets untereinander (`showAll` im Partial, Daten über `preview-sets.11tydata.js`, weil Nunjucks `set`-Variablen nicht in ein Include reicht). Nicht verlinkt, `noindex`, nicht in der Sitemap — **bei der Umstellung entfernen** (Seite, Datendatei, `showAll`-Zweige und das `noindex`-Flag in `base.njk`).
