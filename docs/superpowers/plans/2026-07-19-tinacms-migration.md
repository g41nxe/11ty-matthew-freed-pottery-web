...
**Corrections discovered during Wave 0/1 execution (all committed on `feat/tinacms-migration`; superseding anything below that conflicts):**
- **Single-file collections use `path` = the shared parent directory + `match: { include: "<basename-no-extension>" }`, never a literal full file path.** Tina's `path` must be a directory it scans; Exemplar A already showed this correctly (`path: "src/views"`, `match: { include: "home" }`), but Task 1.8's own collection table (below) originally implied `path: "src/views/home.md"` — that phrasing is wrong, ignore it wherever it still reads that way. All 10 page collections share `path: "src/views"`; `global`/`seo` share `path: "src/views/_data"`.
- **Four collections' internal Tina `name` differs from the plan's naive dashed/short form** — GraphQL rejects dashes in names (alphanumeric + underscore only) and separately reserves `collection`/`collections`/`node`/`document`/`getOptimizedQuery` as built-in Query fields. Use these exact internal `name` strings (the TS export/binding names — `eventsPage`, `faqPage`, `faqSections`, `collectionsPage` — are unchanged):
  - `eventsPage` collection → `name: "events_page"` (not `"events-page"`; that string is reserved for the `events` folder collection)
  - `faqPage` collection → `name: "faq_page"` (not `"faq-page"`)
  - `faqSections` collection → `name: "faq_sections"` (not `"faq-sections"`)
  - `collectionsPage` collection → `name: "collections_page"` (not `"collections"` — collides with Tina's reserved `collections` Query field)
- **`TinaUserCollection` must be imported from `"tinacms-authjs/dist/tinacms"`**, not the bare `"tinacms-authjs"` specifier — it isn't exported from that package's main entry.
- **Stub/placeholder fields must never be named `__placeholder` or anything else starting with `__`** — GraphQL's spec reserves that prefix for introspection (`__typename`, `__schema`, …) and rejects any other field/type using it; this breaks the *entire* runtime schema, not just the offending collection. Use `placeholder` (no underscores) for any remaining scaffolding field names.
- **The frontmatter-preservation assumption above is now empirically confirmed, not just architectural.** A real local GraphQL mutation against `home.md` proved `layout`, `permalink`, `newsletter_flush`, `eleventyNavigation`, and every other undeclared block survive a save byte-for-byte. Expect this exact, harmless cosmetic churn on every Tina save and don't mistake it for a bug: (1) the one schema-declared field relocates to the **end** of the frontmatter block; (2) string scalars containing a comma or a leading protocol (`https://…`) get re-quoted with single quotes even if originally bare; (3) an extra trailing blank line appears after the closing `---`. This was proven for markdown top-level frontmatter only — JSON collections with nested objects (e.g. `global.json`'s `socialmedia`) must still model every key explicitly; don't assume nested-object preservation without testing it.
- **Local verification must use `netlify dev`, not `npm run dev`.** `npm run dev` alone never serves `/api/tina/gql` anywhere reachable — Tina's own dev-server middleware only intercepts the literal path `/graphql`, never the `contentApiUrlOverride`-configured `/api/tina/gql`. `netlify-cli` is now installed as a devDependency specifically so `netlify dev` works without a first-run download.
- **A real `tsconfig.json` + `npm run typecheck` (`tsc --noEmit`, `strict: true`) now exists**, covering `tina/**/*.ts` and `netlify/functions/**/*.ts`. Every Wave 2 task should run `npm run typecheck` as part of its verification — `tinacms build`/Netlify's bundler both use esbuild, which transpiles without type-checking, so a compiling build has never meant a type-correct one.
- **`tina/database.ts` and `netlify/functions/tina.ts` validate required env vars via `requireEnv()` (`tina/util/env.ts`)** before constructing any credentialed client, throwing one combined error listing every missing var. This only runs on the non-local branch — `TINA_PUBLIC_IS_LOCAL=true` needs no credentials and triggers no validation.
...
- [ ] **Step 1: Stub template.** For each collection name in the list below, create `tina/collections/<name>.ts`:
  ...
  ```
  Collections (binding name → internal `name` → `path` + `match.include` → format → kind). See Global Constraints above for why `path` is a shared directory rather than a literal file path, and why 4 internal names differ from their binding name.
  - Pages (single-file, `ui.router` off, create/delete false), all sharing `path: "src/views"`, all `format: "md"`:
    `home`→`name:"home"`, `match:"home"` · `about`→`name:"about"`, `match:"about"` · `pottery`→`name:"pottery"`, `match:"pottery"` · `process` (exported as `process_`)→`name:"process"`, `match:"process"` · `contact`→`name:"contact"`, `match:"contact"` · `collectionsPage`→**`name:"collections_page"`**, `match:"collections"` · `retail`→`name:"retail"`, `match:"retail-stores"` · `privacy`→`name:"privacy"`, `match:"privacy-statement"` · `eventsPage`→**`name:"events_page"`**, `match:"events"` · `faqPage`→**`name:"faq_page"`**, `match:"faq"`.
  - Config singletons (`format: "json"`), sharing `path: "src/views/_data"`: `global`→`name:"global"`, `match:"global"` · `seo`→`name:"seo"`, `match:"seo"`.
  - Folder collections (`create/delete: true`, `path` = content dir, no `match` needed — the whole directory is the collection), `format: "md"`: `events`→`path:"src/content/events"` · `news`→`path:"src/content/news"` · `gallery`→`path:"src/content/gallery"` · `features`→`path:"src/content/features"` · `faqSections`→`path:"src/content/faq"`, **`name:"faq_sections"`**.
  - Auth: `User` via `import { TinaUserCollection } from "tinacms-authjs/dist/tinacms"` (not the bare `"tinacms-authjs"` specifier) re-exported directly in the barrel — not a stub file of its own.
...
- [ ] **Step 1: BLOCKED ON USER — Provision MongoDB Atlas M0**, get `MONGODB_URI`. Generate `NEXTAUTH_SECRET` (`openssl rand -base64 32`). Create the fine-grained PAT (repo Contents R/W). Put all in Netlify env vars for the branch deploy context, with `TINA_PUBLIC_IS_LOCAL=false`. Requires the account owner — not something an agent can or should do.
- [x] **Step 2: Implement `tina/collections/seo.ts`** for `seo.json` fields (`title`, `description`, `url`, `author`, `image`, `options {titleDivider}`). Set `format: "json"`, single file. **Done — `image` is a plain string field, not `{url,alt}`** (the real `seo.json` stores a bare path string; the `{url,alt}` shape here was inherited from the already-stale old Decap `config.yml` and was wrong even before this migration).
- [x] **Step 3: Local round-trip — done, via a documented workaround.** `netlify dev` was not yet installed when this ran; verified instead with a standalone script wiring the exact same `netlify/functions/tina.ts` code directly (Express + `TinaNodeBackend` + `LocalBackendAuthProvider`, no `serverless-http` wrapper). Real GraphQL query → mutate `title` → confirmed on disk → mutate back → confirmed on disk. `netlify-cli` is now installed as a devDependency, so a re-run can use the originally-intended `netlify dev` path if desired.
- [ ] **Step 4: BLOCKED ON USER (needs Step 1's credentials) — Deploy-preview round-trip.** Push branch; on the Netlify deploy preview open `/admin-tina`; log in as `dan` (forced password change); edit an SEO field; **confirm a commit lands on `feat/tinacms-migration`** with exactly that change. This is the one leg of the gate not yet proven against a real GitHub commit — the local round-trip (Step 3) used the same serialization code path, so it should hold, but confirm.
- [x] **Step 5: Frontmatter fidelity check — done, DEFINITIVE YES.** Temporarily added a real `title` field to `home`'s stub schema, mutated it via the same local backend as Step 3, confirmed `layout`, `permalink`, `newsletter_flush`, and `eleventyNavigation` all survived byte-for-byte, alongside every other undeclared block (`hero`, `trust`, `featured_piece`, etc.). Reverted the temporary schema/content changes before committing (only `tina/collections/seo.ts` has a lasting change from this step). Real, harmless cosmetic churn observed and now documented in Global Constraints above (field reorders to the end, comma/protocol-triggered re-quoting, trailing blank line) — expect it, it isn't data loss.
- [ ] **Step 6: GATE DECISION — pending Steps 1 and 4.** Login + field preservation are already proven (locally); the one remaining leg is a real GitHub-committed round-trip once credentials exist. Also discovered and fixed during this spike, retroactively, before Wave 2: all 16 other stub collections shared the reserved field name `__placeholder` (see Global Constraints), which broke the entire runtime schema, not just one collection — this was blocking any collection's round-trip test, not only `seo`'s, and is now fixed everywhere.
